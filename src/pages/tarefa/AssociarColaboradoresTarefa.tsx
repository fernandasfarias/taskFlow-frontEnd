...
import { listarColaboradores } from "../../services/atividadeService";
import { associarColaboradorTarefa } from "../../services/tarefaService";
...
export default function AssociarColaboradoresTarefa() {
  ...
  async function handleAdicionarColaborador(idColaborador) {
    if (!idTarefa || !idColaborador) {
      setErro("ID da tarefa ou do colaborador não encontrado.");
      return;
    }
    ...
    try {
      await associarColaboradorTarefa(idTarefa, idColaborador);

      setColaboradoresAdicionados((prev) => new Set([...prev, idColaborador]));
    } catch (e) {
      console.error(e);
      setErro("Erro ao associar colaborador à tarefa.");
    } finally {
      ...
      <div className="w-full max-w-3xl border border-[#1e2a4a] rounded-2xl p-10">
        <div className="mb-7">
          ...
          <p className="text-sm text-[#64748b] mt-1">
            Busque e adicione colaboradores para esta tarefa
          </p>
        </div>
