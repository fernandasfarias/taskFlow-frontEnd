...
export default function Autenticacao() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0E17] flex items-end md:items-center justify-center md:p-6 font-sans overflow-hidden relative">
      ...
      <div className="absolute inset-0 bg-cover bg-center md:hidden" style={{ backgroundImage: `url('${IMG_FUNDO}')` }} />
        ...
        <div className="text-white">
          ...
          <div className="w-full text-center mt-6">
            <span className="text-base font-normal">
              Não tem uma conta? <button onClick={() => { setEhCadastro(true); setIntroMobile(false); }} className="font-bold hover:underline">Cadastre-se.</button>
            </span>
          </div>
        </div>
      </div>

      <div className={`w-full max-w-[1280px] bg-[#161B22] border border-gray-800 relative overflow-hidden flex text-white z-20 transition-transform duration-500 ease-in-out rounded-t-[40px] md:rounded-[56px] lg:rounded-[64px] h-[88vh] md:h-[90vh] md:min-h-[650px] md:max-h-[850px] ${introMobile ? 'translate-y-[150%] md:translate-y-0' : 'translate-y-0'}`}>
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(enviarFormulario)} className="w-full flex flex-col gap-4 text-white">
        ...
        <button type="submit" className="w-full bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-xl font-bold py-4 mt-2 rounded-2xl md:rounded-3xl transition duration-300 shadow-md">
          Entrar
        </button>
        <div className="w-full text-center mt-4">
          <span className="text-sm md:text-base font-normal text-[#98928A]">
            Não tem uma conta? <button type="button" onClick={aoClicarCadastrar} className="text-white hover:underline font-bold hover:text-[#A78BFA] transition-colors mt-1">Cadastre-se.</button>
          </span>
        </div>
      </form>
    </>
...
function FormularioCadastro({ aoClicarLogin }: FormProps) {
  ...
  const enviarFormulario = async (dados: any) => {
    try {
      ...
      // 2. Realiza o login automático para gerar o token JWT
      const loginResponse = await login(dados.email, dados.senha);
      localStorage.setItem('token', loginResponse.token);

      // 3. Redireciona de acordo com o tipo (sem passar ID na URL, pois agora usamos o token)
      switch(response.tipo){
        case "PROJECT_MANAGER": navigate("/onboarding/certificacoes"); break;
        case "CLIENTE": navigate("/onboarding/empresa"); break;
        case "COLABORADOR": navigate("/onboarding/especialidades"); break;
      ...
      <form onSubmit={handleSubmit(enviarFormulario)} className="w-full flex flex-col gap-4 text-white">
        ...
        <div>
          <div className="relative">
            <select {...register("tipo", { required: "Selecione um tipo!" })} defaultValue="" className="w-full placeholder-[#98928A] bg-transparent text-base md:text-lg py-4 md:py-4 px-6 rounded-2xl md:rounded-3xl border-[2px] border-solid border-[#30363D] focus:border-[#7C3AED] focus:outline-none transition-colors font-normal text-[#98928A] appearance-none cursor-pointer">
          ...
          {errors.tipo && <span className="text-red-500 text-xs mt-1 ml-4 block font-normal">{String(errors.tipo.message)}</span>}
        </div>
        ...
        <div className="w-full text-center mt-4">
          <span className="text-sm md:text-base font-normal text-[#98928A]">
            Já tem uma conta? <button type="button" onClick={aoClicarLogin} className="text-white hover:underline font-bold hover:text-[#A78BFA] transition-colors mt-1">Entre nela.</button>
          </span>
        </div>
      </form>
    </>
...
function Input({ placeholder, type, name, register, errors, regras }: any) {
  return (
    <div>
      <input placeholder={placeholder} type={type} {...register(name, regras || { required: `Obrigatório!` })} className="w-full placeholder-[#98928A] bg-transparent text-base md:text-lg py-4 md:py-4 px-6 rounded-2xl md:rounded-3xl border-[2px] border-solid border-[#30363D] focus:border-[#7C3AED] focus:outline-none transition-colors font-normal text-white" />
      {errors[name] && <span className="text-red-500 text-xs mt-1 ml-4 block font-normal">{String(errors[name].message)}</span>}
    </div>
  );
}